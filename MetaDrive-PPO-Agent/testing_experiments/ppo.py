...
def train_ppo(actor, critic, actor_optimizer, critic_optimizer, obs_batch, act_batch, adv_batch, rtg_batch, config):
    obs_batch = torch.from_numpy(np.array(obs_batch, dtype=np.float32)).to(actor.fc1.weight.device)
    act_batch = torch.from_numpy(np.array(act_batch, dtype=np.float32)).to(actor.fc1.weight.device)
    adv_batch = torch.from_numpy(np.array(adv_batch, dtype=np.float32)).to(actor.fc1.weight.device)
    rtg_batch = torch.from_numpy(np.array(rtg_batch, dtype=np.float32)).to(actor.fc1.weight.device)
